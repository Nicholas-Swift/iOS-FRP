...
class ViewController: UIViewController, BoardViewDelegate {
    ...
    func tappedField(xPosition: Int, yPosition: Int) {
        ...
        let playerWithCurrentTurn = board.value.playerWithCurrentTurn()
        
        // update model and view
        if let moveToRemove = board.value.move(player: playerWithCurrentTurn, x: xPosition, y: yPosition) {
            boardView.clearFieldAtPosition(xPosition: moveToRemove.0, yPosition: moveToRemove.1)
        }
        boardView.updateFieldAtPosition(player: playerWithCurrentTurn, xPosition: xPosition, yPosition: yPosition)
    ...
    @IBAction func restartButtonPressed(_ sender: AnyObject) {
        addBoard?(board.value) // add board
        board.value = Board()
