...
extension Reactive where Base: BoardView {
    ...
    var fieldStates: AnyObserver<[FieldView.State]> {
        
        return UIBindingObserver(UIElement: self.base) { (boardView: BoardView, fieldStates: [FieldView.State]) in
            for (i, fieldState) in fieldStates.enumerated() {
                boardView.fields[i].state = fieldState
            }
            }.asObserver()
    }
