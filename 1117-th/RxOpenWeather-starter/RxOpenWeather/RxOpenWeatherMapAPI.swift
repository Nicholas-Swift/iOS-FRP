...
class RxOpenWeatherMapAPI {
    ...
    func createWeatherObservable(for city: String, temperatureUnit: RxOpenWeatherMapAPI.TemperatureUnit = .fahrenheit) -> Observable<Weather?> {
        ...
        // Create a weatherObservable based on json
        let weatherObservable: Observable<Weather?> = jsonObservable.map { (json: Any) in
            if let jsonDict = json as? [String: Any] {
                let weather = self.jsonToMaybeWeather(weatherInfo: jsonDict)
                return weather
            }
            else {
                return nil
